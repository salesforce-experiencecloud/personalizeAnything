    ...
    @track propInputs = {
        ...
        */
            doNotUseLightningFormattedRichText: {
                ...
                help: 'If checked the component will dynamically insert the html instead of using the Lightning Formatted Rich Text component', //tooltip / help text used for html lightning-input tag
        ...
        for (let key in this.propInputs) {
            ...
            if(generalUtils.objectHasProperty(this.propInputs, key) && this.propInputs[key].doSetDefaultValue === true)
            {
                ...
                if(this.propInputs[key].value !== tmpVal)
                {
                    if(key === 'htmlMarkup')
                    {
                        if(!generalUtils.isStringEmpty(this.htmlMarkup))
                        {
